...
export default function useImageGenProviders(addToast, assistantId) {
		...
		const load = async () => {
			try {
				const [providersRes, settingsRes] = await Promise.all([
					api.get(route('image-gen-providers.index')),
				...
				setActiveModelId(settings.image_gen_model_id ?? null);
			} catch (e) {
				addToast('Failed to load image generation providers', 'error');
			} finally {
				setIsLoading(false);
			}
		};
	...
	const saveProvider = async (index) => {
		...
		try {
			...
			let res;
			if (provider.id) {
				res = await api.patch(route('image-gen-providers.update', { id: provider.id }), payload);
			} else {
				payload.api_key = provider.api_key || '';
				res = await api.post(route('image-gen-providers.store'), payload);
			}
	...
	const deleteProvider = async () => {
		...
		try {
			if (provider.id) {
				const res = await api.delete(route('image-gen-providers.destroy', { id: provider.id }));
	...
	const saveModel = async (providerIndex, modelIndex) => {
		...
		try {
			...
			if (model.id) {
				res = await api.patch(
					route('image-gen-models.update', { provider: provider.id, model: model.id }),
					payload
				);
			} else {
				res = await api.post(
					route('image-gen-models.store', { provider: provider.id }),
					payload
				);
			}
		...
		try {
			if (model.id) {
				const res = await api.delete(
					route('image-gen-models.destroy', { provider: provider.id, model: model.id })
				);
				if (!res.ok) throw new Error('Delete failed');
			}
			setProviders((prev) =>
	...
	const selectModel = async (modelId) => {
		try {
			await api.put(route('settings.selectImageGenModel', { assistant: assistantId }), {
				image_gen_model_id: modelId,
			});
