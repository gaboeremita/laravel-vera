...
## Playback: looping, per-user volume, and mute

**Decision**: Render a single `<audio>` element (or Web Audio node) scoped to the active world session, set to loop, sourced from the world's track URL when present. Volume/mute state lives in component/local state (or `localStorage` for persistence across sessions), never synced from a prop via an effect — toggling mute in a keydown handler and volume via a slider's `onChange`, both applied directly to the audio element.
