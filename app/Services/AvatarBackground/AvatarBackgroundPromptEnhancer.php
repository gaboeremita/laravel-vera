...
class AvatarBackgroundPromptEnhancer
{
    ...
    private const TASK_INSTRUCTION = <<<'PROMPT'
        ...
        The second prompt describes the wide surrounding environment as seen from that location — the walls, windows, architecture, and scenery of the place itself, filling the entire frame from eye-level upward — framed from a standing person's eye-level, looking straight ahead at a natural, level, horizontal angle, at a comfortably close distance that fills the frame with detail rather than a wide establishing shot of the whole room. Add things in the background, never foreground. In a landscape aspect ratio of about 3:2 (roughly 1.57:1, wider than tall). Populate it exactly as the setting itself calls for — a crowd if it's a busy public place, one or two other people if that fits, or nobody at all if it's private or solitary — matching what's actually happening in the conversation right now. This is the view from the observer's own eyes, so whoever appears is someone other than the observer and other than the character themselves, since both are already accounted for separately. The floor this environment sits on is handled by a separate image entirely, so this one stays focused on what's at eye-level and above.
