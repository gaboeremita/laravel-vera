...
class TalkToTool extends WorldTool
{
    ...
    {
        return 'Starts talking to someone in person: the user, or another resident of this world by name. You talk from where you are when they are close enough to talk to, and walk over to them only when they are farther away, saying your opening line when you get there. With a resident you have talked to before, your conversation picks up where it stopped.';
    }
    ...
    {
        return [...($this->toolbox->userAvailable ? [WorldToolbox::USER_TARGET] : []), ...array_keys($this->toolbox->companions ?? [])];
    }
    ...
    public function handle(array $arguments): array
    {
        ...
        if ($residentId === null) {
            throw new RuntimeException(sprintf('There is nobody called "%s" here. You can talk to: %s.', $target, implode(', ', $this->targets())));
        }
