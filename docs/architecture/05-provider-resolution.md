...
| World resident LLM | Same as LLM, for conversations and self-chosen decisions in a world with layout markers | Model must have `supports_tools`; an NPC without a selected model uses the config fallback | NPCs without a selected model: `AI_DEFAULT_*` | 422 before loop, and residents without one cannot be placed |
