...
- [X] T020 [P] [US1] **(added post-analysis — `/speckit-analyze` finding C1)** Feature test `tests/Feature/ImageGenerationToolEmptyPromptTest.php`: a `generate_image` call with an empty `prompt` argument is rejected (FR-008) without ever calling the image-gen provider and without creating a carrier message/image. FR-008 previously had implementation (T008) but no dedicated test, mirroring an existing, pre-analysis gap in `BasicCalculatorTool`'s own empty-argument guard from 001-agentic-task-loop.
...
- **Post-Implementation Fixes (Phase 7)**: Found after Phase 6 was believed complete, via live testing and `/speckit-analyze` — T018 depends on User Story 1 (T008) already existing; T019 depends on Foundational (T004) already existing.
...
- T006/T007/T020 (US1's test files) can be written in parallel once Foundational is done.
