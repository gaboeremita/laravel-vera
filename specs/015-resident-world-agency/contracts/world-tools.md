...
An activity may name the pose it plays. `use`, `zone` and plan steps take an optional `pose` from her own library, so she can match an activity to her pose of another name ("prepare_drink" for "Mix a drink"). Without one, the activity's own pose plays when she has a pose of that name, compared case-insensitively with `_`, `-` and spaces alike; otherwise she holds her default stance. The chosen pose is returned in the action as `pose`.
