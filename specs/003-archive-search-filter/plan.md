...
| VIII. State Derivation Happens During Render | GATE — see below | The technical brief's suggested pattern (`vectorResultIds` state set from a debounced fetch effect) is fine as written — that's legitimate async-effect state. But the *merged, deduplicated, ranked display list* must be computed in the render body from `entries` + `searchQuery` + `vectorResultIds`, not stored via a second `useState` synced through another effect. Flagged explicitly for the tasks phase. |
